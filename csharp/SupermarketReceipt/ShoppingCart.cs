        ...
        public void HandleOffers(Receipt receipt, Dictionary<Product, Offer> offers, SupermarketCatalog catalog)
        {
            foreach (var p in _productQuantities.Keys)
            {
                ...
                if (offers.ContainsKey(p))
                {
                    ...
                    {
                        var numberOf3s = quantityAsInt / 3;
                        var discountAmount = quantity * unitPrice - (numberOf3s * 2 * unitPrice + quantityAsInt % 3 * unitPrice);
                    ...
                    {
                        var numberOf5s = quantityAsInt / 5;
                        var discountTotal = unitPrice * quantity - (offer.Argument * numberOf5s + quantityAsInt % 5 * unitPrice);
                        discount = new Discount(p, 5 + " for " + PrintPrice(offer.Argument), -discountTotal);
                    }
